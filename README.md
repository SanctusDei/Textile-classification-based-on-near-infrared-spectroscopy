...
> **Note**: k=5 and k=10 share identical teacher baselines (0.9657) because they use the same 5 seeds. k=3 uses a 3-seed subset (teacher 0.9573). The L1-penalized student **exceeds the teacher at all k values** — embedded L1 regularization provides beneficial model selection that the full-spectrum teacher cannot match, likely because the teacher overfits on small per-fold training sets (228 features, ~28 samples).
